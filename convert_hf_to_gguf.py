...
class _Qwen35MRopeMixin:
    ...
    def set_gguf_parameters(self):
        super().set_gguf_parameters()  # pyright: ignore[reportAttributeAccessIssue] # ty: ignore[unresolved-attribute]
        ...
        if name.startswith("mtp."):
            ...
            if name.find("layers.") != -1:
                ...
                for b in range(n_layer, self.block_count):
                    yield from super().modify_tensors(data_torch, tmpl.format(bid=b), b)  # pyright: ignore[reportAttributeAccessIssue]
                return

        yield from super().modify_tensors(data_torch, name, bid)  # pyright: ignore[reportAttributeAccessIssue]
